...
class Steganography:
    ...
    def uint16_to_indexed_hex(self, uint16_no, index):
        """
    ...
    def convert_uint16_to_list_of_uint8(self, uint16_no):
        """
        ...
        """
        return [self.uint16_to_indexed_hex(uint16_no, index) for index in range(0, 8)]

    def _add_secret_file_size_into_base_image(self, image, width, height):
        """
        ...
        image[0, 0:8, 0] = image[0, 0:8, 0] | self.convert_uint16_to_list_of_uint8(np.uint16(width))
        # Adding Height Inside Image
        image[0, 0:8, 1] = image[0, 0:8, 1] | self.convert_uint16_to_list_of_uint8(np.uint16(height))
        return image

    def add_secret_image_inside_base_image(self, base_image, secret_image):
        """
        ...
        """
        base_image = self._add_secret_file_size_into_base_image(base_image, secret_image.shape[1], secret_image.shape[0])
    ...
    def _get_steganography_file_size(self, image):
        """
    ...
    def get_secret_image_inside_base_image(self, base_img):
        """
        ...
        [height, width] = self._get_steganography_file_size(base_img)
    ...
    def embed_image_into_another_image(self):
        """
        ...
        while True:
            try:
                ...
                # Reading Image
                base_image = imread(base_image_location)
        ...
        while True:
            try:
                ...
                # Reading Image
                secret_img = imread(secret_image_location)
        ...
        base_image = self.add_secret_file_size_into_base_image(base_image, secret_img.shape[1], secret_img.shape[0])

        # Embedding whole secret image into the base image
        embedded_image = self.add_secret_image_inside_base_image(base_image, secret_img)
        # Getting Name of Result image
        embedded_image_name = input(
            "Please Enter A Name For Steganography Image(Just Name Dont Add Location): ") + ".png"
        ...
        imwrite(embedded_image_full_path, embedded_image)

    def extracting_secret_image_from_embedded_image(self):
        while True:
            try:
                embedded_image_location = input("Please Enter Absolute Path to Embedded Image: ")
                embedded_image = imread(embedded_image_location)
                break
            except:
                print("Please Enter Valid Image Location")
        [height, width] = self.get_steganography_file_size(embedded_image)

        secret_image = self.get_secret_image_inside_base_image(embedded_image, width, height)

        # Getting Name of secret image
        secret_image_name = input(
            "Please Enter A Name For Secret Image(Just Name Dont Add Location): ") + ".png"
        # File Location(path) where image will store
        ...
        # Writing image to given location with given name
        imwrite(secret_image_full_path, secret_image)
